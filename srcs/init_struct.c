...
void	ft_init_struct_floor_sky(t_cub *cub)
{
	...
	door_locked = malloc(sizeof(t_texture));
	key = malloc(sizeof(t_texture));
	potion = malloc(sizeof(t_texture));
	if (!sky || !floor || !sprite || !flashlight || !door || !door_locked || !key || !potion)
		ft_error(MALLOC);
	cub->sky = sky;
	cub->floor = floor;
	cub->sprite = sprite;
